    ...
    return (
        <div className='sm:max-w-7xl mx-auto px-2 mt-10'>
            <form onSubmit={submitJobApplication} className='shadow-xl w-2xl mx-auto px-10 py-10 rounded-2xl'>
                ...
                <button disabled={!verifyForm()} onClick={() => { setShowURL(false); verifyForm() }} className={`bg-blue-600 px-4 py-2 w-full text-white mt-4 rounded-full ${verifyForm() ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}>
                    Submit Application
                </button>
            </form>
        </div>
